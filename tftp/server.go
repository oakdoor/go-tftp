...
func (s *Server) newConn(req *request, reqChan chan []byte) (*conn, func() error, error) {
	...
	if s.singlePort {
		c = newSinglePortConn(req.addr, dg.mode(), s.conn, reqChan, s.userFullWindowReAckDeadline)
	} else {
		c, err = newConn(s.net, dg.mode(), req.addr, 0, s.userFullWindowReAckDeadline) // Use empty mode until request has been parsed.
		if err != nil {
...
func FullWindowReAckDeadline(deadlineMs int) ServerOpt {
	return func(s *Server) error {
		if deadlineMs >= 0 {
			...
			*fullWindowReAckDeadline = time.Duration(deadlineMs) * time.Millisecond
			s.userFullWindowReAckDeadline = fullWindowReAckDeadline
		} else {
			s.userFullWindowReAckDeadline = nil
		}
		return nil
	}
}
